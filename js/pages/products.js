...
var productHolders = Array.from(document.querySelectorAll(".products"));
var productsArray;
var selectedImage = document.getElementById("selected-image");
var privewProduct = document.getElementById("privew");
...
function applyFunctions() {
  productHolders.forEach(function (ele) {
    var holder = ele;
    if (loaded) {
...
function displayFakeProducts(productsHolder) {
  ...
  var holder = document.querySelector(".".concat(uniecClass, " .holder .row"));
  var prodNums = 1;
  ...
  for (var i = 0; i < prodNums; i++) {
    var fakeProduct = document.createElement("div");
    ...
    fakeProduct.addEventListener("click", function () {
      return false;
    });
  }
}
...
function addShowingProductsNumber(productsHolder) {
  ...
  showMoreBtn === null || showMoreBtn === void 0 || showMoreBtn.addEventListener("click", function () {
    ...
    if (nextShowingProducts >= productsLength) showMoreBtn.classList.add("d-none");
...
function showClickedPictureMoreDetails(productsHolder) {
  var uniecClass = productsHolder.getAttribute("data-un-class");
  var products = document.querySelectorAll(".".concat(uniecClass, " .holder .row"));
  var emptyArea = document.getElementById("low-opacity-area");
  if (productsHolder.classList.contains("empty")) return false;
  products.forEach(function (card) {
    card.addEventListener("click", function (clickedProduct) {
      if (card.classList.contains("fake-product")) {
        return false;
      }
      var clickedElement = clickedProduct.target;
      ...
      if (clickedElement.tagName == "IMG") {
        var privewImage = clickedElement;
        selectedImage.src = privewImage.src;
        var productDiv = clickedElement.parentNode;
        productId = productDiv.getAttribute("prod-id") || "";
      } else if (clickedElement.tagName === "DIV") {
        if (clickedElement.classList.contains("product")) {
          var _privewImage = clickedElement.children[0];
          selectedImage.src = _privewImage.src;
        }
        productId = clickedElement.getAttribute("prod-id") || "";
      }
      var productData = productsData.filter(function (prod) {
        return prod.id == productId;
      })[0];

      // Privew Product Detailes

      var prodType = document.getElementById("prod-type");
      var prodModel = document.getElementById("prod-model");
      var prodDescription = document.getElementById("prod-description");
  ...
  emptyArea === null || emptyArea === void 0 || emptyArea.addEventListener("click", function () {
    if (!privewProduct.classList.contains("h-0")) {
...
function addMoreButtonControl(productsHolder) {
  var uniecClass = productsHolder.getAttribute("data-un-class");
  var products = document.querySelectorAll(".".concat(uniecClass, " .holder .row .product"));
  var showMoreBtn = document.querySelector(".".concat(uniecClass, " .show-more"));
  if (products.length <= 5) {
    showMoreBtn === null || showMoreBtn === void 0 || showMoreBtn.classList.add("d-none");
  } else {
    showMoreBtn === null || showMoreBtn === void 0 || showMoreBtn.classList.remove("d-none");
  }
}
